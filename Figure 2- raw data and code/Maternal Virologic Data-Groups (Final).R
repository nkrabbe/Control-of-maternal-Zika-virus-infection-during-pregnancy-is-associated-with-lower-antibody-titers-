...
### 
plasma_vRNA_burden_duration_group <- read_excel("R:\\Antibody and Cellular Assays\\Antibody Work\\Correlates of Protection (P1 Animals)\\Maternal Viral Characteristics\\Final Raw Data, figures, and code\\Maternal Virologic Data (Final).xlsx", sheet = 6, col_names = TRUE)
...
###
plasma_infectious_virus_duration_group <- read_excel("R:\\Antibody and Cellular Assays\\Antibody Work\\Correlates of Protection (P1 Animals)\\Maternal Viral Characteristics\\Final Raw Data, figures, and code\\Maternal Virologic Data (Final).xlsx", sheet = 7, col_names = TRUE)
...
### 
peak_plasma_vRNA_group <-  read_excel("R:\\Antibody and Cellular Assays\\Antibody Work\\Correlates of Protection (P1 Animals)\\Maternal Viral Characteristics\\Final Raw Data, figures, and code\\Maternal Virologic Data (Final).xlsx", sheet = 4, col_names = TRUE)
...
peak_plasma_vRNA_group_figure <- 
  ...
  theme(axis.title= element_text(size = 20, face = "bold"), axis.text = element_text(size = 20)) + 
  theme(aspect.ratio = 1) + scale_fill_manual(values = c("black", "orange"), breaks = c("Controller", "Non-Controller")) + scale_color_manual(values = c("black", "orange"), breaks = c("Controller", "Non-Controller")) + 
...
###
vRNA_infectivity_ratio_group <- read_excel("R:\\Antibody and Cellular Assays\\Antibody Work\\Correlates of Protection (P1 Animals)\\Maternal Viral Characteristics\\Final Raw Data, figures, and code\\Maternal Virologic Data (Final).xlsx", sheet = 5, col_names = TRUE)
...
###
AUC_group <-  read_excel("R:\\Antibody and Cellular Assays\\Antibody Work\\Correlates of Protection (P1 Animals)\\Maternal Viral Characteristics\\Final Raw Data, figures, and code\\Maternal Virologic Data (Final).xlsx", sheet = 2, col_names = TRUE)
...
###
mfi_vRNA_burden_group <- read_excel("R:\\Antibody and Cellular Assays\\Antibody Work\\Correlates of Protection (P1 Animals)\\Maternal Viral Characteristics\\Final Raw Data, figures, and code\\Maternal Virologic Data (Final).xlsx", sheet = 8, col_names = TRUE)
mfi_vRNA_burden_group$Percent_vRNA_Positive_Biopsies = as.numeric(mfi_vRNA_burden_group$Percent_vRNA_Positive_Biopsies)
...
mfi_vRNA_burden_group_figure <- 
  ggplot(data = mfi_vRNA_burden_group, aes(x = Maternal_Fetal_Interface_Tissue, y = Percent_vRNA_Positive_Biopsies)) + 
  ...
  theme_classic() + scale_y_continuous(name = "Biopsies vRNA-Positive (% Positive)", limits = c(0, 60), breaks = c(0, 15, 30, 45, 60)) + 
...
mfi_vRNA_burden_group_controller_chorionic_quantile <- quantile(mfi_vRNA_burden_group_controller_chorionic$Percent_vRNA_Positive_Biopsies, probs = c(0.25, 0.75), na.rm = TRUE)
...
mfi_vRNA_burden_group_controller_decidua_quantile <- quantile(mfi_vRNA_burden_group_controller_decidua$Percent_vRNA_Positive_Biopsies, probs = c(0.25, 0.75), na.rm = TRUE)
...
mfi_vRNA_burden_group_controller_placenta_quantile <- quantile(mfi_vRNA_burden_group_controller_placenta$Percent_vRNA_Positive_Biopsies, probs = c(0.25, 0.75), na.rm = TRUE)
...
mfi_vRNA_burden_group_controller_total_quantile <- quantile(mfi_vRNA_burden_group_controller_total$Percent_vRNA_Positive_Biopsies, probs = c(0.25, 0.75), na.rm = TRUE)
...
mfi_vRNA_burden_group_noncontroller_chorionic_quantile <- quantile(mfi_vRNA_burden_group_noncontroller_chorionic$Percent_vRNA_Positive_Biopsies, probs = c(0.25, 0.75), na.rm = TRUE)
...
mfi_vRNA_burden_group_noncontroller_decidua_quantile <- quantile(mfi_vRNA_burden_group_noncontroller_decidua$Percent_vRNA_Positive_Biopsies, probs = c(0.25, 0.75), na.rm = TRUE)
...
mfi_vRNA_burden_group_noncontroller_placenta_quantile <- quantile(mfi_vRNA_burden_group_noncontroller_placenta$Percent_vRNA_Positive_Biopsies, probs = c(0.25, 0.75), na.rm = TRUE)
...
mfi_vRNA_burden_group_noncontroller_total_quantile <- quantile(mfi_vRNA_burden_group_noncontroller_total$Percent_vRNA_Positive_Biopsies, probs = c(0.25, 0.75), na.rm = TRUE)
...
summary_stats <-
  ...
  summarize(median = median(Percent_vRNA_Positive_Biopsies, na.rm = TRUE), IQR = IQR(Percent_vRNA_Positive_Biopsies, na.rm = TRUE), count = n())
chorionic_plate <- filter(mfi_vRNA_burden_group, Maternal_Fetal_Interface_Tissue == 'Chorionic Plate')
wilcox.test(Percent_vRNA_Positive_Biopsies~Virologic_Control_Group, data = chorionic_plate, exact = FALSE, na.rm = TRUE)
decidua <- filter(mfi_vRNA_burden_group, Maternal_Fetal_Interface_Tissue == 'Decidua')
wilcox.test(Percent_vRNA_Positive_Biopsies~Virologic_Control_Group, data = decidua, exact = FALSE, na.rm = TRUE)
placenta <- filter(mfi_vRNA_burden_group, Maternal_Fetal_Interface_Tissue == 'Placenta')
wilcox.test(Percent_vRNA_Positive_Biopsies~Virologic_Control_Group, data = placenta, exact = FALSE, na.rm = TRUE)
total <- filter(mfi_vRNA_burden_group, Maternal_Fetal_Interface_Tissue == 'Total')
wilcox.test(Percent_vRNA_Positive_Biopsies~Virologic_Control_Group, data = total, exact = FALSE, na.rm = TRUE)
...
ggarrange(AUC_group_figure, plasma_vRNA_burden_duration_group_figure, mfi_vRNA_burden_group_figure, peak_plasma_vRNA_group_figure, plasma_infectious_virus_duration_group_figure, vRNA_infectivity_ratio_group_figure, labels= c('A', 'B', 'C', 'D', 'E', 'F'), nrow= 2, ncol = 3, align= 'v', common.legend = TRUE, legend = 'top')
dev.off()
